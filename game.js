...
class MainGameScene extends Phaser.Scene {
  ...
  update(time, delta) {
    ...
    this.doors.getChildren().forEach(door => {
      if (!door.isOpen) return; // Skip closed doors
      ...
      if (d < closestDist) {
        ...
        // Auto-transition when crossing threshold
        if (this.isCrossingDoorThreshold(door)) {
          const [nx, ny] = door.targetRoom.split(",").map(Number);
          console.log(`→ automatically moving to room ${nx},${ny}`);
    ...
    setupPathfindingGrid() {
      ...
      this.doors.getChildren().forEach(door => {
        if (this.handleDoorInteraction(door)) {
          // Door was entered, no need to process more doors
          return;
        }
      });
