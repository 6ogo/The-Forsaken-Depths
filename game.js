...
class TitleScene extends Phaser.Scene {
    ...
    create() {
      ...
      this.add.text(400, 260, 'WASD to move, Arrows/Mouse to shoot', { font: '18px Arial', fill: '#ffffff' }).setOrigin(0.5);
  
      const start = this.add.text(400, 360, 'Start Game', { font: '32px Arial', fill: '#00ff00' })
  ...
  class MainGameScene extends Phaser.Scene {
    ...
    preload() {
      ...
      const assets = [
        ...
        'boss_projectile','blob_projectile','gold_sparkles',
        'heart_full','heart_half','heart_empty','background'
      ];
    ...
    update(time) {
      ...
      if (!this.inTransition) {
        ...
        this.doors.children.iterate(door => {
          ...
          if (dist < 40) { nearDoor = true; this.handleDoorPrompt(door); }
        });
    ...
    generateWorldMap() {
      ...
      if (normals.length) this.roomMap[Phaser.Utils.Array.GetRandom(normals)].type = 'shop';
      for (let i=0; i<3; i++) {
        const keys = Object.keys(this.roomMap);
        ...
        for (const d of dirs) {
          ...
          if (!this.roomMap[key] && Object.keys(this.roomMap[base].doors).length < 3) {
            this.roomMap[key] = { type:'normal', doors:{}, depth:this.roomMap[base].depth+1, variation:Phaser.Math.Between(0,2) };
    ...
    loadRoom(x, y) {
      ...
      ['up','down','left','right'].forEach(dir => this.createWallSegments(dir, !!this.roomMap[key].doors[dir]));
  
      // Room type
      switch(this.roomMap[key].type) {
        case 'shop': this.createShopRoom(); break;
        case 'boss': if (!this.clearedRooms.has(key)) this.createBossRoom(); break;
        case 'normal': if (!this.clearedRooms.has(key)) this.createNormalRoom(); break;
      }
    ...
    createRoomLayout(v) {
      ...
      if (v === 0) [[200,200],[600,200],[200,400],[600,400]].forEach(p => this.innerWalls.create(p[0],p[1],wkey).setScale(2));
      if (v === 1) { this.innerWalls.create(400,300,wkey).setScale(4,1); this.innerWalls.create(400,300,wkey).setScale(1,4); }
      if (v === 2) { this.innerWalls.create(300,200,wkey).setScale(2).setAngle(45); this.innerWalls.create(500,400,wkey).setScale(2).setAngle(45); }
    }
    ...
    openAllDoors() {
      this.doors.children.iterate(d => {
        d.setTexture('door');
        d.isOpen = true;
        if (d.collider) { this.physics.world.removeCollider(d.collider); d.collider = null; }
      });
    }
    ...
    handleDoorPrompt(door) {
      ...
      this.doorPrompt.setText(canOpen? 'Press E to open door': '').setVisible(true);
      if (canOpen && this.keys.E.isDown && !door.isOpen) {
        door.setTexture('door'); door.isOpen = true;
    ...
    isCrossing(door) {
      ...
      switch(door.direction) {
        case 'up': return p.y <= door.y+15 && this.keys.W.isDown;
        case 'down': return p.y >= door.y-15 && this.keys.S.isDown;
        case 'left': return p.x <= door.x+15 && this.keys.A.isDown;
        case 'right': return p.x >= door.x-15 && this.keys.D.isDown;
    ...
    setupColliders() {
      ...
      // door colliders
      this.doors.children.iterate(door => { if (!door.isOpen) door.collider = this.physics.add.collider(this.player, door); });
    }
    ...
    onBossDefeated() {
      ...
      if (this.currentWorld < this.maxWorlds) {
        ...
        this.generateWorldMap();
        const msg = this.add.text(400, 300, `Entering World ${this.currentWorld}!`, { font: '32px Arial', fill: '#ffffff', backgroundColor: '#000000' })
          .setOrigin(0.5).setDepth(200);
        this.time.delayedCall(2000, () => { msg.destroy(); this.loadRoom(0, 0); });
      } else {
        const victory = this.add.text(400, 300, 'Victory! You have conquered The Forsaken Depths!', { font: '24px Arial', fill: '#ffffff', backgroundColor: '#000000' })
          .setOrigin(0.5).setDepth(200);
  ...
  class GameOverScene extends Phaser.Scene {
    ...
    create() {
      ...
      const again = this.add.text(400, 350, 'Play Again', { font: '32px Arial', fill: '#00ff00' })
        .setOrigin(0.5).setInteractive();
      ...
      const exit = this.add.text(400, 450, 'Exit', { font: '32px Arial', fill: '#00ff00' })
        .setOrigin(0.5).setInteractive();
