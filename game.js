...
class MainGameScene extends Phaser.Scene {
  ...
  update(time, delta) {
    ...
    // --- Door Interactions ---
    // This is the key part - check all doors using the helper method
    this.doors.getChildren().forEach(door => {
      if (this.handleDoorInteraction(door)) {
        // If transition happened, no need to check other doors
        return;
      }
    });
  ...
  setupPathfindingGrid() {
    ...
    // Mark closed doors as unwalkable (NOT using handleDoorInteraction here!)
    this.doors.getChildren().forEach(door => {
      if (!door.isOpen) {
